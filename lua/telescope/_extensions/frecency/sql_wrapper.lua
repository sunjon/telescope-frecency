...
local queries = {
  ["file_add_entry"]              = "INSERT INTO files (path, count) values(?, 1);",
  ["file_update_counter"]         = "UPDATE files SET count = count + 1 WHERE path = ?;",
  ["timestamp_add_entry"]         = "INSERT INTO timestamps (file_id, timestamp) values(?, julianday('now'));",
  ["timestamp_delete_before_id"]  = "DELETE FROM timestamps WHERE id < ? and file_id == ?;",
  ...
  ["get_row"]                     = "SELECT * FROM files WHERE path == ?;",
  ["get_timestamp_ids_for_file"]  = "SELECT id FROM timestamps WHERE file_id == ?;",
}
...
function M:bootstrap(opts)
  ...
  -- create the db object if it doesn't exist
  local db_root = opts.docs_root or "$XDG_DATA_HOME/nvim"
  local db_filename = db_root .. "/file_frecency.sqlite3"
  self.db = sql.new(db_filename)
  if not self.db then print("error") return end

  -- create tables if they don't exist
  if vim.loop.fs_stat(db_filename) == nil then -- make sense?
...
function M:do_transaction(query, params)
  ...
  return self.db:with_open(function(db)
    return db:eval(queries[query], params)
  end)
end

function M:get_row_id(filepath)
  local result = self:do_transaction('get_row', filepath)
...
function M:update(filepath)
  ...
  if not file_id then
    self:do_transaction('file_add_entry', filepath)
    file_id = self:get_row_id(filepath)
  else
  -- ..or update existing entry
    self:do_transaction('file_update_counter', filepath)
  end

  -- register timestamp for this update
  self:do_transaction('timestamp_add_entry', file_id)

  -- trim timestamps to MAX_TIMESTAMPS per file (there should be up to MAX_TS + 1 at this point)
  local timestamps = self:do_transaction('get_timestamp_ids_for_file', file_id)
  local trim_at = timestamps[(#timestamps - MAX_TIMESTAMPS) + 1]
  if trim_at then
    self:do_transaction('timestamp_delete_before_id', { trim_at.id, file_id })
  end
end
